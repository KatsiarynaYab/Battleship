...
import config
from controllers.button import YesNoButton


class GameOverWindow:
    def __init__(self):
        self.__surf = pygame.Surface(config.game_over_window_size)
        self.visible = False
    ...
    def update(self, screen):
        if self.visible:
            self.__surf.fill(config.bg_color)
            ...
            self.__surf.blit(result_sprite, result_rect)
            self.__surf.blit(text_sprite, text_rect)
            screen.blit(self.__surf, config.game_over_window_coordinates)
